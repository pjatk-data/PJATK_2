...
def evaluate_confidence(extract_result: dict, choice: Choice, model: str = "gpt-4o"):
    ...
    # Retrieves the specific encoding used for the model (if available).
    # On platforms where `tiktoken` can't be installed (e.g., Windows ARM64 without Rust),
    # we fall back to a simple token-length heuristic. This is sufficient for mapping
    # OpenAI logprob tokens to character offsets in the generated text.
    encoding = tiktoken.encoding_for_model(model) if tiktoken is not None else None
