...
from confidence.confidence_utils import get_confidence_values
from utils.value_utils import value_contains, value_match
from concurrent.futures import ThreadPoolExecutor, as_completed

from utils.image import scale_flat_poly_list
...
def normalize_polygon(page: DocumentPage, polygon: list[float]) -> list[dict[str, int]]:
    """
...
def extract_lines(
    analyze_result: AnalyzeResult, multiple_score_resolver: callable = min
) -> list[DIDocumentLine]:
    """
        ...
        for line in page.lines:
            ...
            di_line = DIDocumentLine(
                **line_copy,
                ...
                confidence=multiple_score_resolver(contained_words_conf_scores),
                # normalized_polygon=normalize_polygon(page, line_copy.polygon)
...
def evaluate_confidence(extract_result: dict, analyze_result: AnalyzeResult):
    """
    ...
    Args:
        ...
        generated_fields: Optional set of field names that should be excluded from confidence calculation
                          as they are generated programmatically and not directly extracted from the document.
    ...
    def evaluate_field_value_confidence(
        ...
        if isinstance(value, dict):
            ...
            return {
                ...
                "value": value,
                "page_numbers": [line.page_number for line in matching_lines],
            }
    ...
    # Calculate confidence scores excluding generated fields
