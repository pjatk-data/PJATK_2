...
class VehicleInsuranceCostDetails(BaseModel):
    ...
    #         annual_total=0, payable_by_date=datetime.now().strftime("%Y-%m-%d")
    #     )
...
class VehicleInsurancePersonDetails(BaseModel):
    ...
    address: Optional[str] = Field(description="The current address of the person.")
...
class VehicleInsuranceVehicleDetails(BaseModel):
    ...
    make: Optional[str] = Field(description="The make of the vehicle.")
    model: Optional[str] = Field(description="The model of the vehicle.")
    year: Optional[int] = Field(description="The year the vehicle was manufactured.")
    value: Optional[float] = Field(description="The current value of the vehicle.")
    ...
    #         registration_number="", make="", model="", year=2024, value=0.0
    #     )
...
class VehicleInsuranceExcessDetails(BaseModel):
    ...
    compulsory: Optional[int] = Field(description="The compulsory excess amount.")
    voluntary: Optional[int] = Field(description="The voluntary excess amount.")
    ...
    #         compulsory=0, voluntary=0, unapproved_repair_penalty=0
    #     )
    ...
    def from_json(json_str: str):
        ...
        cost = (
            VehicleInsuranceCostDetails(
                cost.get("annual_total", None), cost.get("payable_by_date", None)
            )
            if cost is not None
            else None
        )
        ...
        renewal = (
            VehicleInsuranceRenewalDetails(
                renewal.get("renewal_notification_date", None),
                renewal.get("last_date_to_renew", None),
            )
        ...
        policyholder = (
            VehicleInsurancePersonDetails(
                policyholder.get("first_name", None),
                policyholder.get("last_name", None),
                policyholder.get("date_of_birth", None),
                policyholder.get("address", None),
                policyholder.get("email_address", None),
                policyholder.get("total_years_of_residence_in_uk", None),
                policyholder.get("driving_license_number", None),
            )
        ...
        vehicle = (
            VehicleInsuranceVehicleDetails(
                vehicle.get("registration_number", None),
                vehicle.get("make", None),
                vehicle.get("model", None),
                vehicle.get("year", None),
        ...
        accident_excess = (
            VehicleInsuranceExcessDetails(
                accident_excess.get("compulsory", None),
                accident_excess.get("voluntary", None),
                accident_excess.get("unapproved_repair_penalty", None),
            )
            if accident_excess is not None
            else None
        )

        fire_and_theft_excess = (
            VehicleInsuranceExcessDetails(
                fire_and_theft_excess.get("compulsory", None),
                fire_and_theft_excess.get("voluntary", None),
                fire_and_theft_excess.get("unapproved_repair_penalty", None),
            )
            if fire_and_theft_excess is not None
            else None
        )
